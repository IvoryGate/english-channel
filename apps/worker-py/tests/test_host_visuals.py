...
def test_scene_prompt_includes_fixed_hosts_and_no_text_rule() -> None:
    ...
    cover_prompt = bundle["coverImagePrompt"]
    background_prompt = bundle["videoBgImagePrompt"]
    for prompt in (cover_prompt, background_prompt):
        assert "Riley" in prompt
        assert "Sam" in prompt
        assert "left" in prompt
        assert "right" in prompt
    assert "no text" in background_prompt.lower()
    assert "ENGLISH ALONE EVERY DAY" in cover_prompt
