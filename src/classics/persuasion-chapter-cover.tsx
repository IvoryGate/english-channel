...
const chapters = {
  1: {
    ...
    summary: 'At Kellynch Hall, appearances are everything.',
  },
  2: {
    background: 'chapter-02-cover-bg-v1.png',
    hook: ['THE PRICE', 'OF PRIDE'],
    summary: 'Kellynch Hall can no longer afford appearances.',
  },
  3: {
    background: 'chapter-03-cover-bg-v1.png',
    hook: ['A NAME', 'RETURNS'],
...
export const PersuasionChapterCover = ({chapter}: {chapter: 1 | 2 | 3}) => {
  ...
  return (
    <AbsoluteFill style={{backgroundColor: '#f3dfb7', color: '#4c3328', overflow: 'hidden'}}>
      <Img
        src={staticFile(`classics/persuasion/${details.background}`)}
        style={{position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover'}}
      />
      <AbsoluteFill style={{background: 'linear-gradient(90deg, rgba(255,249,232,.94) 0%, rgba(255,246,222,.76) 38%, rgba(255,246,222,.05) 62%, rgba(87,55,34,.08) 100%)'}} />
      ...
      <div style={{position: 'absolute', left: 126, top: 110, width: 1070}}>
        ...
        <div style={{fontFamily: serif, fontSize: 82, lineHeight: 1.04, fontWeight: 700, letterSpacing: 1}}>{details.hook[0]}<br />{details.hook[1]}</div>
        <div style={{fontFamily: serif, fontSize: 31, lineHeight: 1.35, fontStyle: 'italic', color: '#76584b', marginTop: 22, maxWidth: 930}}>{details.summary}</div>
        <div style={{display: 'flex', gap: 24, alignItems: 'center', marginTop: 58}}>
          <div style={{fontFamily: serif, fontSize: 34, letterSpacing: 7, border: '2px solid #b78636', padding: '15px 22px', color: '#4c3328', background: 'rgba(255,252,242,.72)'}}>CHAPTER {chapter}</div>
