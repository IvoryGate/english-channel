...
def normalize_generated_cover(source_image: Path, output_jpg: Path, *, mode: str = "blur-fill-composite", safe_margin: float = 0.04) -> Path:
    from normalize_youtube_cover import normalize_youtube_cover

    output_jpg.parent.mkdir(parents=True, exist_ok=True)
    # blur-fill-composite / contain: pass safe_margin so edge text gets breathing room.
    # blur-fill / auto: no margin needed (background fill, no sharp text layer).
    margin = safe_margin if mode in ("contain", "blur-fill-composite") else 0.0
    normalize_youtube_cover(source_image, output_jpg, mode=mode, safe_margin=margin)
    return output_jpg
...
def prepare_outputs_from_generated(
    *,
    ...
    normalize_generated_cover(cover_source, cover_jpg, mode="blur-fill-composite", safe_margin=0.04)
