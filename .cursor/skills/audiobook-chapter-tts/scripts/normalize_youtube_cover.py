...
- ``blur-fill-composite``: ``blur-fill`` backdrop + the sharp cover **height-filled**
  on top with **rounded corners** and **feathered edges** (no top/bottom gap).
  This is the default for **episode covers/thumbnails**: the 16:9
...
Defaults are tuned for ELR dialogue-series covers: blur_radius=24 (soft backdrop),
height-fill sharp layer (no top/bottom gap), corner_radius=48, feather=16. See
...
def _blur_fill(image: Image.Image, width: int, height: int, blur_radius: float = 24.0) -> Image.Image:
...
def _blur_fill_composite(
    ...
    """Blurred fill background + sharp cover with rounded, feathered edges.

    The sharp layer scales to **fill frame height** (no top/bottom gap). When the
    scaled width exceeds the canvas, center-crop horizontally. Blurred fill only
    shows on the left/right when the source is narrower than 16:9.
    """
    ...
    if new_w > width:
        left = (new_w - width) // 2
        sharp = sharp.crop((left, 0, left + width, height))
        new_w = width
    else:
    ...
    canvas.alpha_composite(sharp, (paste_x, 0))
    ...
    *,
    ...
    with Image.open(input_path) as image:
        ...
        if _is_target_size((src_w, src_h), width, height):
            ...
            normalized = _blur_fill(rgb, width, height, blur_radius=blur_radius)
        elif mode == "blur-fill-composite":
            normalized = _blur_fill_composite(
                rgb, width, height, blur_radius=blur_radius, safe_margin=safe_margin
            )
